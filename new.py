...
# Tek dosya yükleyici
uploaded_file = st.file_uploader("CSV Dosyasını Yükleyin", type=["csv"], key="main_csv")
...
if uploaded_file is not None:
    ...
    selected_name = st.selectbox("Öğrenci Seçin", student_names, key="select_student")

    subjects = df[df["name"] == selected_name]["subject"].unique()
    selected_subject = st.selectbox("Ders Seçin", subjects, key="select_subject")
    ...
    if not student_df.empty:
        ...
        if submitted:
            if from_email and password:
                # Mail gönderme fonksiyonu (aynı dosyada tanımlı olmalı)
                ...
                to_email = student_df["email"].iloc[0]
